    ...
    convenience init(_ attribute: AttributeSyntax) {
        ...
        attribute.arguments?.description
          ...
          .forEach { (idx, part) in
              ...
              default:
                  logger.astError("Unrecognized attribute format \(attribute.arguments?.description ?? "")")
                  return
              }
          }

        self.init(name: attribute.attributeName.trimmedDescription, arguments: arguments, description: attribute.trimmedDescription)
    }
