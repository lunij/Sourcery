...
public final class FileParserSyntax: SyntaxVisitor, FileParserType {
    ...
    public init(contents: String, forceParse: [String] = [], parseDocumentation: Bool, path: Path? = nil, module: String? = nil) throws {
        ...
        modifiedDate = path.flatMap { (try? FileManager.default.attributesOfItem(atPath: $0.string)[.modificationDate]) as? Date }
        self.module = module
    ...
    public func parse() throws -> FileParserResult {
        // Inline handling
        let inline = TemplateAnnotationsParser.parseAnnotations("inline", contents: initialContents, forceParse: forceParse)
