...
private extension Networking {
    ...
    func XAppTokenRequest(_ defaults: UserDefaults) -> Observable<String?> {
        ...
        let newTokenRequest = provider.request(ArtsyAPI.xApp)
...
extension Networking {
    ...
    func request(_ token: ArtsyAPI, defaults: UserDefaults = UserDefaults.standard) -> Observable<Moya.Response> {
        ...
        return XAppTokenRequest(defaults).flatMap { _ in actualRequest }
    }
}

extension AuthorizedNetworking {
    func request(_ token: ArtsyAuthenticatedAPI, defaults _: UserDefaults = UserDefaults.standard) -> Observable<Moya.Response> {
...
extension NetworkingType {
    static func newDefaultNetworking() -> Networking {
        Networking(provider: newProvider(plugins))
    }

    static func newAuthorizedNetworking(_ xAccessToken: String) -> AuthorizedNetworking {
        AuthorizedNetworking(provider: newProvider(authenticatedPlugins, xAccessToken: xAccessToken))
    }

    static func newStubbingNetworking() -> Networking {
        Networking(provider: OnlineProvider(endpointClosure: endpointsClosure(), requestClosure: Networking.endpointResolver(), stubClosure: MoyaProvider.ImmediatelyStub, online: .just(true)))
    }

    static func newAuthorizedStubbingNetworking() -> AuthorizedNetworking {
        AuthorizedNetworking(provider: OnlineProvider(endpointClosure: endpointsClosure(), requestClosure: Networking.endpointResolver(), stubClosure: MoyaProvider.ImmediatelyStub, online: .just(true)))
    }

    static func endpointsClosure<T>(_ xAccessToken: String? = nil) -> (T) -> Endpoint<T> where T: TargetType, T: ArtsyAPIType {
        return { target in
            var endpoint: Endpoint<T> = .init(URL: url(target), sampleResponseClosure: { .networkResponse(200, target.sampleData) }, method: target.method, parameters: target.parameters)
    ...
    static func APIKeysBasedStubBehaviour(_: some Any) -> Moya.StubBehavior {
        APIKeys.sharedKeys.stubResponses ? .immediate : .never
    }
...
private func newProvider<T>(_ plugins: [PluginType], xAccessToken: String? = nil) -> OnlineProvider<T> where T: TargetType, T: ArtsyAPIType {
    OnlineProvider(
        endpointClosure: Networking.endpointsClosure(xAccessToken),
